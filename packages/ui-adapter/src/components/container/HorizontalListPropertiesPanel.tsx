...
const DATA_MODE_OPTIONS: { value: HorizontalListDataMode; label: string }[] = [
...
const findChildByRole = (
  ...
  for (const childId of childIds) {
    ...
    if (childProps?.horizontalListRole === role) {
      return childId;
    }
  }
...
const findItemImageNodeIds = (pageId: string, nodeId: string): string[] => {
  ...
  return editorApi
    ...
    .filter((childId) => editorApi.getNodeProps(childId)?.horizontalListRole === 'item')
    .map((itemNodeId) =>
      editorApi
        .getChildNodeIds(itemNodeId)
        .find(
          (childId) => editorApi.getNodeProps(childId)?.horizontalListRole === 'item-image'
        ) ?? null
    )
    .filter((childId): childId is string => Boolean(childId));
};

export const HorizontalListPropertiesPanel = () => {
  ...
  const dataMode = (props.dataMode as HorizontalListDataMode) ?? 'expression';
  ...
  const height = Number(props.height ?? 280);
  ...
  const normalizeStructure = React.useCallback(() => {
    ...
    editorApi.getChildNodeIds(itemsNodeId).forEach((itemNodeId) => {
      ...
      if (itemProps?.horizontalListRole !== 'item') return;

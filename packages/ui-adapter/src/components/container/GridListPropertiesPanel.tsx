...
const DATA_MODE_OPTIONS: { value: GridListDataMode; label: string }[] = [
...
const findChildByRole = (
  ...
  for (const childId of childIds) {
    ...
    if (childProps?.gridListRole === role) {
      return childId;
    }
  }
...
export const GridListPropertiesPanel = () => {
  ...
  const title = String(props.title ?? '这是一个标题');
  const columnCount = Number(props.columnCount ?? 2);
  const columnGap = Number(props.columnGap ?? 20);
  const rowGap = Number(props.rowGap ?? 20);
  const dataMode = (props.dataMode as GridListDataMode) ?? 'expression';
  ...
  const height = Number(props.height ?? 560);
  ...
  const normalizeStructure = React.useCallback(() => {
    ...
    editorApi.getChildNodeIds(itemsNodeId).forEach((itemNodeId) => {
      ...
      if (itemProps?.gridListRole !== 'item') return;
  ...
  const updateItemsLayout = (patch: Record<string, unknown>) => {
    const editorApi = getEditorApi(activeNode.pageId);
    const itemsNodeId = findChildByRole(activeNode.pageId, activeNode.nodeId, 'items');
    if (!editorApi || !itemsNodeId) return;

    const itemsProps = editorApi.getNodeProps(itemsNodeId);
    const layout = (itemsProps?.layout as Record<string, unknown> | undefined) ?? {
      arrangement: 'wrap',
      spacingMode: 'fixed',
      spacing: 20,
  ...
  return (
    <section className="space-y-0">
      ...
      <CollapsibleSection title="常用设置">
        ...
        <div className="flex items-center gap-2 py-1.5">
          <span className="w-14 shrink-0 text-xs text-slate-600">列数量</span>
          <div className="flex flex-1 items-center">
            <InputNumber
              value={columnCount}
              onChange={(value) => {
                const nextValue = Math.max(1, value ?? columnCount);
                updateProp('columnCount', nextValue);
                updateItemsLayout({ columnCount: nextValue });
              }}
              size="small"
              min={1}
              className="flex-1"
        ...
        <div className="flex items-center gap-2 py-1.5">
          <span className="w-14 shrink-0 text-xs text-slate-600">列距值</span>
          <div className="flex flex-1 items-center">
            <InputNumber
              value={columnGap}
              onChange={(value) => {
                const nextValue = Math.max(0, value ?? columnGap);
                updateProp('columnGap', nextValue);
                updateItemsLayout({ columnGap: nextValue, spacing: nextValue });
              }}
              size="small"
              min={0}
              className="flex-1"
        ...
        <div className="flex items-center gap-2 py-1.5">
          <span className="w-14 shrink-0 text-xs text-slate-600">行距值</span>
          <div className="flex flex-1 items-center">
            <InputNumber
              value={rowGap}
              onChange={(value) => {
                const nextValue = Math.max(0, value ?? rowGap);
                updateProp('rowGap', nextValue);
                updateItemsLayout({ rowGap: nextValue });
              }}
              size="small"
              min={0}
              className="flex-1"

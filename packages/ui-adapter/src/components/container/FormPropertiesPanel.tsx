...
const FORM_LAYOUT_OPTIONS: { value: InputFieldLayout; label: string }[] = [
  { value: 'vertical', label: '上下' },
  { value: 'horizontal', label: '左右' },
];

const FORM_LAYOUT_TIPS = `可整体控制表单中的表单项的标签与输入区的布局方式。表单和表单项同时设置两个不同的布局方式时，以表单项的设置为主。`;
...
export const FormPropertiesPanel = () => {
  ...
  // 常用设置
  const formLayout = (p.formLayout as InputFieldLayout) ?? 'vertical';
  ...
  // 尺寸
  const widthMode = (p.widthMode as SizeMode) ?? 'fill';
  const heightMode = (p.heightMode as SizeMode) ?? 'fit-content';
  ...
  }, [heightMode, overflow, updateProp]);
      ...
      <CollapsibleSection title="常用设置">
        ...
        <div className="flex items-center gap-2 py-1.5">
          <div className="flex shrink-0 items-center gap-1">
            <span className="whitespace-nowrap text-xs text-slate-600">表单布局</span>
            <InfoTip content={FORM_LAYOUT_TIPS} />
          </div>
          <div className="w-28 shrink-0 ml-auto">
            <SegmentedControl
              ...
              value={formLayout}
              onValueChange={(v) => {
                updateProp('formLayout', v);

                // 联动更新子组件的 layout
                if (!activeNode) return;
                const editorApi = getEditorApi(activeNode.pageId);
                if (!editorApi) return;
                const childIds = editorApi.getChildNodeIds(activeNode.nodeId);
                const FORM_ITEM_NAMES = new Set(['InputField', 'NumberInput', 'RadioGroup', 'CheckboxGroup']);
                for (const childId of childIds) {
                  const name = editorApi.getNodeComponentName(childId);
                  if (name && FORM_ITEM_NAMES.has(name)) {
                    editorApi.updateNodeProp(childId, 'layout', v);
                  }
                }
              }}
              columns={2}

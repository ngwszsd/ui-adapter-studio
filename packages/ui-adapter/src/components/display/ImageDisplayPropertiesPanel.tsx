...
import { Minus, Plus, Trash2, Upload, FileImage } from 'lucide-react';
...
const SOURCE_TYPE_OPTIONS: { value: ImageSourceType; label: string }[] = [
  { value: 'local', label: '本地上传' },
  { value: 'binding', label: '绑定数据' },
];

const FILL_MODE_OPTIONS: { value: ImageFillMode; label: string }[] = [
...
const SOURCE_TIP = '可本地上传也可通过workflow绑定数据，本地上传支持 .png .jpg 等格式，不支持 .svg 格式';

const FILL_MODE_TIP = `覆盖：图片可能会被裁剪，确保容器内没有空白
...
export const ImageDisplayPropertiesPanel = () => {
  ...
  // 常用设置
  const sourceType = (p.sourceType as ImageSourceType) ?? 'local';
  const imageUrl = (p.imageUrl as string | null) ?? null;
  const bindingField = String(p.bindingField ?? '');
  const fillMode = (p.fillMode as ImageFillMode) ?? 'cover';
  ...
  const height = Number(p.height ?? 120);
  ...
  const fileName = imageUrl
    ? decodeURIComponent(imageUrl.split('/').pop()?.split('?')[0] ?? '图片')
    : null;
  ...
  return (
    <section className="space-y-0">
      ...
      <CollapsibleSection title="常用设置">
          ...
          <div className="flex-1">
            <SegmentedControl
              ...
              onValueChange={(v) => {
                ...
                if (v === 'binding') updateProp('imageUrl', null);
              }}
              columns={2}
            />
          </div>
          ...
          <div className="py-1.5">
            {imageUrl ? (
              <div className="flex items-center gap-2 rounded-md border border-slate-200 bg-slate-50 px-2 py-1.5">
                ...
                <button
                  type="button"
                  onClick={() => updateProp('imageUrl', null)}
        ...
        {sourceType === 'binding' && (
          <div className="flex items-center gap-2 py-1.5">
            ...
            <div className="flex-1">
              <TemplateInput
                value={bindingField}
                onChange={(v) => updateProp('bindingField', v)}
                placeholder="请输入绑定字段"
                picker={picker}
              />
            </div>
        ...
        {/* 填充方式 */}
        <div className="flex items-center gap-2 py-1.5">
          <span className="shrink-0 text-xs text-slate-600">填充方式</span>
          <InfoTip content={FILL_MODE_TIP} />
          <Select
            value={fillMode}
            onValueChange={(v) => updateProp('fillMode', v)}
          >

...
const INPUT_TYPE_OPTIONS: { value: NumberInputType; label: string }[] = [
  { value: 'number', label: '数字' },
  { value: 'password', label: '密码' },
];

const DATA_TYPE_OPTIONS: { value: NumberDataType; label: string }[] = [
...
export const NumberInputPropertiesPanel = () => {
  ...
  const label = String(p.label ?? '数字输入');
  const required = (p.required as boolean) ?? false;
  const placeholder = String(p.placeholder ?? '请输入数字');
  const inputType = (p.inputType as NumberInputType) ?? 'number';
  const dataType = (p.dataType as NumberDataType) ?? 'integer';
  const decimalPlaces = Number(p.decimalPlaces ?? 2);
  const step = Number(p.step ?? 1);
  const defaultValue = p.defaultValue as number | string | null ?? null;
  ...
  return (
    <section className="space-y-0">
      ...
      <CollapsibleSection title="常用设置">
        ...
        <div className="flex items-center gap-2 py-1.5">
          ...
          <div className="flex-1">
            <SegmentedControl
              ...
              value={inputType}
              onValueChange={(v) => updateProp('inputType', v)}
              columns={2}
            />
          </div>
        </div>
        ...
        {inputType === 'number' && (
          <div className="flex items-center gap-2 py-1.5">
            <span className="w-14 shrink-0 text-xs text-slate-600">数据类型</span>
            <div className="flex-1">
              <SegmentedControl
                options={DATA_TYPE_OPTIONS}
                value={dataType}
                onValueChange={(v) => updateProp('dataType', v)}
                columns={2}
              />
            </div>
          </div>
        ...
        {inputType === 'number' && dataType === 'decimal' && (
          <div className="flex items-center gap-2 py-1.5">
            <span className="w-14 shrink-0 text-xs text-slate-600">小数位数</span>
            <InputNumber
              value={decimalPlaces}
              onChange={(v) => updateProp('decimalPlaces', v && v > 0 ? v : 1)}
              size="small"
              min={1}
              max={10}
              className="flex-1"
        ...
        {inputType === 'number' && (
          <div className="flex items-center gap-2 py-1.5">
            <span className="w-14 shrink-0 text-xs text-slate-600">步长值</span>
            <InputNumber
              value={step}
              onChange={(v) => updateProp('step', v ?? 1)}
              size="small"
              min={0.01}
              step={dataType === 'decimal' ? 0.1 : 1}
              className="flex-1"
            />
          </div>
        )}
          ...
          {inputType === 'password' ? (
            <Input
              ...
              onChange={(e) => {
                ...
                updateProp('defaultValue', next === '' ? null : next);
              }}
              className="h-7 flex-1 text-xs"
            />
          ) : (
            <InputNumber
              value={
                typeof defaultValue === 'number'
                  ? defaultValue
                  : typeof defaultValue === 'string' && defaultValue.trim() !== '' && !Number.isNaN(Number(defaultValue))
                    ? Number(defaultValue)
                    : undefined
              }
              onChange={(v) => updateProp('defaultValue', v ?? null)}
              size="small"
              step={step}
              className="flex-1"
            />
